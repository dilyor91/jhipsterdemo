  ...
  return (
    <div>
      <h2 id="events-heading" data-cy="EventsHeading">
        <Translate contentKey="jhipsterSampleApplicationApp.events.home.title">Events</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="jhipsterSampleApplicationApp.events.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/events/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="jhipsterSampleApplicationApp.events.home.createLabel">Create new Events</Translate>
          </Link>
        </div>
      </h2>
        ...
        {eventsList && eventsList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                <th>
                  <Translate contentKey="jhipsterSampleApplicationApp.events.id">ID</Translate>
                </th>
                <th>
            ...
            <tbody>
              {eventsList.map((events, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{events.titleUz}</td>
                  <td>{events.titleRu}</td>
                  <td>{events.titleKr}</td>
                  <td>{events.contentUz}</td>
                  <td>{events.contentRu}</td>
                  <td>{events.contentKr}</td>
                  <td>{events.postedDate ? <TextFormat type="date" value={events.postedDate} format={APP_LOCAL_DATE_FORMAT} /> : null}</td>
                  <td>{events.status ? 'true' : 'false'}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="jhipsterSampleApplicationApp.events.home.notFound">No Events found</Translate>
            </div>
          )
        )}
      </div>
